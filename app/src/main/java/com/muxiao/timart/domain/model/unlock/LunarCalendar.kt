package com.muxiao.timart.domain.model.unlock

import java.time.LocalDate

/**
 * 公历 → 农历换算（压缩位表法，纯 JVM 可单测，零权限零联网）。
 *
 * 数据为通行的农历压缩位表（1900–2100，每年一个 int）：
 * - 低 4 位：闰月月份（0 = 无闰月）；
 * - 位 0x10 起自高位向低位：闰月及 1–12 月的大小月标记（0x10000 对应闰月，0x8000 对应正月）；
 * - 0x8000..0x10 中 1 的个数 + 闰月天数 = 全年天数。
 * 表值与 hutool / calendar.js 等主流实现同源，2057 年取香港天文台修正值。
 * 支持区间外的日期返回 null（判定侧 fail-closed 不满足）。
 */
object LunarCalendar {

    /** 农历日期（[month] 1–12；[isLeapMonth] = 闰 [month] 月） */
    data class LunarDate(
        val year: Int,
        val month: Int,
        val day: Int,
        val isLeapMonth: Boolean,
    )

    private val LUNAR_INFO = intArrayOf(
        0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950,
        0x16554, 0x056a0, 0x09ad0, 0x055d2, 0x04ae0, 0x0a5b6,
        0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2,
        0x095b0, 0x14977, 0x04970, 0x0a4b0, 0x0b4b5, 0x06a50,
        0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
        0x06566, 0x0d4a0, 0x0ea50, 0x16a95, 0x05ad0, 0x02b60,
        0x186e3, 0x092e0, 0x1c8d7, 0x0c950, 0x0d4a0, 0x1d8a6,
        0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2,
        0x0a950, 0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0,
        0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0,
        0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260,
        0x0f263, 0x0d950, 0x05b57, 0x056a0, 0x096d0, 0x04dd5,
        0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540,
        0x0b6a0, 0x195a6, 0x095b0, 0x049b0, 0x0a974, 0x0a4b0,
        0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
        0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58,
        0x05ac0, 0x0ab60, 0x096d5, 0x092e0, 0x0c960, 0x0d954,
        0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0,
        0x092d0, 0x0cab5, 0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50,
        0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
        0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6,
        0x0a4e0, 0x0d260, 0x0ea65, 0x0d530, 0x05aa0, 0x076a3,
        0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250,
        0x0d520, 0x0dd45, 0x0b5a0, 0x056d0, 0x055b2, 0x049b0,
        0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
        0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6,
        0x0ea50, 0x06aa0, 0x1a6c4, 0x0aae0, 0x092e0, 0x0d2e3,
        0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0,
        0x0a6d0, 0x055d4, 0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0,
        0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
        0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55,
        0x04b60, 0x0a570, 0x054e4, 0x0d160, 0x0e968, 0x0d520,
        0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0,
        0x0d150, 0x0f252, 0x0d520,
    )

    /** 表支持的首日：1900-01-31（农历 1900 年正月初一） */
    private val MIN_EPOCH_DAY = LocalDate.of(1900, 1, 31).toEpochDay()

    /** 表支持的最后一个农历年 */
    private const val MAX_LUNAR_YEAR = 2100

    /** 农历某年总天数（闰年含闰月） */
    private fun lunarYearDays(year: Int): Int {
        val info = LUNAR_INFO[year - 1900]
        var sum = 348
        var mask = 0x8000
        while (mask > 0x8) {
            if (info and mask != 0) sum++
            mask = mask shr 1
        }
        return sum + lunarLeapDays(year)
    }

    /** 农历某年的闰月月份（0 = 无闰月） */
    private fun lunarLeapMonth(year: Int): Int = LUNAR_INFO[year - 1900] and 0xf

    /** 农历某年闰月的天数（无闰月为 0） */
    private fun lunarLeapDays(year: Int): Int {
        if (lunarLeapMonth(year) == 0) return 0
        return if (LUNAR_INFO[year - 1900] and 0x10000 != 0) 30 else 29
    }

    /** 农历某年某月（1–12 正月序，不含闰月）的天数：29 小月 / 30 大月 */
    private fun lunarMonthDays(year: Int, month: Int): Int =
        if (LUNAR_INFO[year - 1900] and (0x10000 shr month) != 0) 30 else 29

    /**
     * 公历日期 → 农历日期；表支持区间（1900-01-31 起至 2100 年末）外返回 null。
     * 经典压缩位表遍历算法：先定位农历年，再逐月扣减定位月日；
     * 月循环退出时指针指向"下一个待定月"，由 offset == 0 / offset < 0 两个收尾分支修正，
     * 闰月插入用 --month 指针回退 + isLeap 标记承载（与 calendar.js 逐行同构）。
     */
    fun solarToLunar(date: LocalDate): LunarDate? {
        var offset = date.toEpochDay() - MIN_EPOCH_DAY
        if (offset < 0) return null

        // 定位农历年：offset 归零后剩余 0 表示当年正月初一
        var year = 1900
        while (year < MAX_LUNAR_YEAR && offset >= lunarYearDays(year)) {
            offset -= lunarYearDays(year)
            year++
        }
        if (offset >= lunarYearDays(year)) return null // 超出表支持范围（2101 年及以后）

        val leapMonth = lunarLeapMonth(year)
        var isLeap = false
        var month = 1
        var monthDays = 0
        while (month <= 12 && offset > 0) {
            if (leapMonth > 0 && month == leapMonth + 1 && !isLeap) {
                // 落到闰月：指针回退，改用闰月天数
                --month
                isLeap = true
                monthDays = lunarLeapDays(year)
            } else {
                monthDays = lunarMonthDays(year, month)
            }
            if (isLeap && month == leapMonth + 1) isLeap = false
            offset -= monthDays
            month++
        }
        if (offset == 0L && leapMonth > 0 && month == leapMonth + 1) {
            if (isLeap) {
                isLeap = false
            } else {
                isLeap = true
                --month
            }
        }
        if (offset < 0) {
            offset += monthDays
            --month
        }

        return LunarDate(year = year, month = month, day = offset.toInt() + 1, isLeapMonth = isLeap)
    }
}
