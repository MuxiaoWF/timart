...
- **39 种解锁条件**，按需组合（「全部满足 AND」/「任意满足 OR」）：
  - **时间**（8 种）：指定日期 / 指定时刻 / 满 N 天 / 满 N 分钟 / 星期几 / 时间段 / 每月几号 / 每年今日
  - **设备状态**（7 种）：电量区间 / 充电状态 / 省电模式 / 静音模式 / 耳机连接 / 网络类型 / Wi-Fi 名称
  - **行为与步数**（5 种）：今日步数 / 连续步数达标 / 累计开启次数 / 连续开启天数 / 距上次开启天数
  - **位置与环境**（9 种）：到达某地 / 离开某地（半径判定）/ 天气类型 / 气温区间 / 湿度·风速·气压·紫外线 / 日出日落时段 / 月相 / 下一闹钟之前 / 运动状态（行走/静止）
  - **胶囊联动**（5 种）：依赖另一颗解锁 / 依赖另一颗销毁 / 胶囊总数达标 / 指南针朝向 / 海拔区间
  - **现场挑战**（5 种）：问答 / 谜题 / 摇一摇 / 翻面静置 / NFC 触碰——打开胶囊时当场完成，不参与自动巡检
- **设备能力门控**：本机硬件不支持的条件（如无气压计的海拔、无 NFC 的触碰）在创建时即不可选并注明原因；导入备份时也会提示哪些条件在本机无法达成
