...
- **109 unlock condition types**, combined with AND ("all must be met"), OR ("any will do"), or M-of-N ("any M of N" — keep a spare key, one missed condition can still open it):
  - **Time** (21): fixed date / fixed date-time / N days elapsed / N minutes elapsed / day of week / time range / monthly day / yearly date / lunar festivals (Chinese New Year, Mid-Autumn…) / solar terms / zodiac season / day length / sunrise-sunset time range / Nth weekday of month / leap day / etc.
  - **Device state** (25): battery level / charging state / power-save mode / silent mode / headphone connected / airplane mode / music playing / before the next alarm / motion state (walking/still) / compass heading / altitude range / dark theme / do-not-disturb / media volume / VPN / battery temperature / orientation / Bluetooth audio device / proximity covered / time since boot / a specific app installed / etc.
  - **Location & environment** (24): arrive at a place / leave a place (radius-based) / weather type / temperature range / humidity·wind·pressure·UV / network type / Wi-Fi SSID / sun phase / golden hour / moon phase / meteor-shower peak night / darkness (light sensor) / another timezone / on the move (GPS speed) / air quality / wind direction / hemisphere / current city / etc.
  - **In-app stats & links** (20): daily step count / step streak / total open count / consecutive open days / days since last open / capsule count reached / another capsule unlocked / destroyed / opened & read / gazed at this capsule N times / total gaze duration / opens today / a backup completed / etc.
  - **Live challenges** (19): question / riddle / shake / flip-and-hold / long-press / biometric (fingerprint/face) / photo keepsake / NFC tap / gesture pattern / voice passphrase / scan a code / hold still / volume-key combo / etc. — completed on the spot when opening the capsule, never counted by automatic checks
...
- **Rituals & expression**: when sealing, record a voice message, sketch a drawing, pick a stationery style — or seal the capsule as a **blind box** (you won't know what's inside until it opens); write **replies** (kept in the dust record), **gift a capsule** to someone else (the passphrase travels with the gift — only the holder can open it), or use an **NFC tag** as a physical anchor (stick it where it belongs, tap to reach the capsule)
- **Capsule structures**: puzzle capsules — multiple parts unlock in order and finally assemble into one complete letter; nested capsules — opening the outer shell awakens the dormant seed inside
- **Ambience & keepsakes**: ambient soundscapes and haptic signatures make unsealing feel real; the star map draws constellation lines and capsules ripen visually as conditions near, with a "draw one for today" pick; an **annual star-map report** poster every year; destroyed capsules keep a "life story" record in the dust archive
...
- **Keepsake poster & data safety**: generate a shareable poster after unlocking, and export a memorial page for dust records too; export everything as a fully encrypted backup archive; optional **automatic local backups on a schedule** — at the interval you choose, encrypted backups are written to a local folder you pick, keeping the most recent few; a **library health check** scans for orphan records, dead dependency links and file integrity; a **cross-device migration wizard** guides the old-device export, new-device import and verification step by step
...
- Automatic backups write only to a local folder you choose on the device; the content is fully encrypted and never touches the network
